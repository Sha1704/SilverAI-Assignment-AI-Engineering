...
def format_context(hits: list[dict]) -> str:
    ...
    blocks: list[str] = []
    for h in hits:
        pages = (h.get("metadata") or {}).get("pages", [])
...
if prompt:
    ...
    # Guard: must have indexed PDF (assignment-aligned)
    ...
    if prompt.strip().lower().startswith("/handbook"):
        ...
        with st.chat_message("assistant"):
            ...
            # Save a SMALL message to chat history (not the whole handbook)
            done_msg = (
        ...
        # Preview outside chat container (so it never looks cut off in chat)
        with st.expander("Handbook preview (first 6000 chars)"):
            st.markdown(st.session_state.latest_handbook_md[:6000])

        # Rerun so the sidebar download button updates immediately
        st.rerun()
...
- Include citations like (PDF p. X) when supported by excerpts.
"""
